...
`LOGSTASH_HOME/data/dead_letter_queue/main` by default. Queues are written to a
sequentially numbered temporary file (`1.log.tmp`, `2.log.tmp`, etc) in this directory,
and renamed to `1.log`, `2.log`, etc, either when this file has stored a set amount of data, or
when the `flush_interval` has elapsed since creation, providing an entry has been
...
You can specify `dead_letter_queue.flush_interval` to set the amount of time between
creating a dead letter queue file and it being made available to reading. It is
